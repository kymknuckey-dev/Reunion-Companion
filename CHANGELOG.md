# FFD 2.0 RC1.0.14.8.9.9.4.1.3.12.10.5 — Improve Media Filename Standards

- Adds **Non-standard filename** to the existing Improve > Data Quality Centre Media status row.
- Uses `Surname, First names [optional descriptive text].extension` as the baseline while leaving descriptive text unrestricted.
- Clicking the status shows the non-conforming filenames in the existing Media browser.
- Removes the redundant lower **Missing media**, **Legacy PICT**, and **Untitled sources** cards.
- Keeps Place variants and Duplicate source titles unchanged.
- Filename review is read-only; Companion does not rename, move, or delete Media files.
...
- Brings Media root selection and reconciliation status cards directly into Improve.
- Replaces full-image delivery with generated 240 px thumbnails for image previews; PDFs retain first-page previews.
- Uses compact status cards and compact media rows while preserving the read-only audit model.
...
## FFD 2.0 RC1.0.14.8.9.9.4.1.3.12.10.5.2 — Current Person External Evidence Indicator
- Keeps the factual Timeline free of unaccepted research candidates.
- In Research mode, the Current Person header highlights unresolved External Evidence already linked to that person.
- Shows the number of new candidates and links directly to External Evidence Review filtered to that person.
- Works whether or not Reunion currently records a Death event.
- Presentation mode and published output remain unchanged.
