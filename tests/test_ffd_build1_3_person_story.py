...
def test_story(tmp_path,monkeypatch):
 monkeypatch.setenv("HOME",str(tmp_path));db=connect(tmp_path/"x.sqlite3");seed(db);set_presentation_mode(True);p=render_get(db,"/person/1",{});assert all(x in p for x in ("A life in the family history","Born 1933","Elaine Fay Cox","Birth note","Overview","Timeline","Biography"))
...
def test_deep_links(tmp_path,monkeypatch):
 monkeypatch.setenv("HOME",str(tmp_path));db=connect(tmp_path/"x.sqlite3");seed(db);set_presentation_mode(True);p=render_get(db,"/person/1",{});assert all(x in p for x in ("tab=timeline","tab=biography","tab=family","tab=media","tab=publish"))
def test_timeline(tmp_path,monkeypatch):
 monkeypatch.setenv("HOME",str(tmp_path));db=connect(tmp_path/"x.sqlite3");seed(db);set_presentation_mode(True);p=render_get(db,"/person/1",{"tab":"timeline","view":"story"});assert all(x in p for x in ("Life Timeline","Birth"))
